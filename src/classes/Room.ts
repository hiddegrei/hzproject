...
export default class Room {
 ...
 // private minigame12: MiniGame12;
  ...
  public timeOutRooms(elapsed: number) {
    for (let i = 0; i < 17; i++) {
      if (this.timeoutRooms[i][1] === true && this.timeoutRooms[i][0] >= Room.TIMEOUT_ROOMS) {
        this.timeoutRooms[i] = [0, false];
      } else {
  ...
  static drawImageScaled(ctx: CanvasRenderingContext2D, img: string, imgWidth: number, imgHeight: number, xPos: number, yPos: number) {
