  ...
  public showRoomIds(room: Room) {
    for (let i = 0; i < this.rooms.length; i++) {
      if(this.rooms[i][2] ==="15"){
        ...
        this.ctx.beginPath();
        this.ctx.arc(this.rooms[i][0], this.rooms[i][1], 10, 0, 2 * Math.PI);
        this.ctx.stroke();
        this.ctx.closePath();
        this.ctx.fill();
        if (this.rooms[i][2] === "90") {
          this.writeTextToCanvas("Shop", 20, this.rooms[i][0], this.rooms[i][1] - 20);
        } else {
          this.writeTextToCanvas(this.rooms[i][2], 20, this.rooms[i][0], this.rooms[i][1] - 20);
        }
      }
    }
  }
