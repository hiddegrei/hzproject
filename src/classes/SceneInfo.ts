...
export default class SceneInfo {
  ...
  public loadAgents(widthHall: number) {
    ...
    // this.agentMid=
    agents.push(new Agent(this.canvas.width / 2 - 0.5 * widthHall, 100 + 4.5 * widthHall, this.ctx, widthHall, "mid", 3, "red"));

    agents.push(new Agent(this.canvas.width / 2 + 12 * widthHall, 100 + 6.5 * widthHall, this.ctx, widthHall, "search11", 3, "orange"));
    return agents;
  }
  ...
  public renderBackgroundImages(widthHall: number, imgBank: HTMLImageElement) {
    ...
    this.writeTextToCanvas("Grote Kluis", 30, this.canvas.width / 2 - 2 * widthHall, 400, "start", "black");

    //draw background central hub
    this.ctx.drawImage(imgBank, 550, 800, 1150, 750, this.canvas.width / 2 - 4 * widthHall, 100 + 5 * widthHall, 7 * widthHall, 4 * widthHall);
  }
  ...
  public renderInfo(timeLeft: number, score: number, progressNum: number, hints: Hints, trans: any) {
    ...
    this.ctx.closePath();
    this.ctx.fill();
    this.writeTextToCanvas(`time left: ${Math.round(timeLeft/1000)}`, 20, 100, 40 - trans.y);
   ...
   * @param alignment
   */
  public writeTextToCanvas(text: string, fontSize: number = 20, xCoordinate: number, yCoordinate: number, alignment: CanvasTextAlign = "start", color: string = "red"): void {
